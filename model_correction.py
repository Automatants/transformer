...
class Head(nn.Module):
    def __init__(self, config: GPT2Config):
        ...
        self.key_getter = nn.Linear(config.n_embd, self.head_dim, bias=False)
        self.query_getter = nn.Linear(config.n_embd, self.head_dim, bias=False)
        self.value_getter = nn.Linear(config.n_embd, self.head_dim, bias=False)
        self.register_buffer("trill", torch.tril(torch.ones(config.block_size, config.block_size)))
    def forward(self, x):
        B, T, C = x.size()
        k = self.key_getter(x)
        q = self.query_getter(x)
        v = self.value_getter(x)
        masked_attn_scores = q @ k.transpose(-2, -1) * (1 / math.sqrt(C))
        masked_attn_scores = masked_attn_scores.masked_fill(self.trill[:T, :T] == 0, float('-inf'))
